% Autor:
% Datum: 20.11.2013   , 2016 auf Aktionen der Vorlesung umgestellt
...
problem :-
  plan(
  [table(a),table(b),table(c),clear(a),clear(b),clear(c)],
  [on(b,c), on(a,b)]).
